...
AI の導入は、単なるツールの追加ではない。それは組織の文化と技術的基盤を問う「組織変革」そのものである。本稿では、DORA が新たに提唱した [AI Capabilities Model](https://dora.dev/ai/capabilities-model/report/) を軸に、AI の恩恵を最大化するための条件を読み解く。重要なのは、この新モデルが従来の [DORA Core Model](https://dora.dev/research/) を置き換えるものではなく、それを補完し、AI という強力なエンジンを制御するためのガイドであるという点にある。
...
ユーザーのニーズという北極星がない状態でのAI活用は、単にゴミを高速で生産する状態、いわゆる機能量産工場化を招く。ユーザー中心の視点が高い組織は AI の加速が直接的な価値創出につながり、チームパフォーマンスが大幅に向上する。
ユーザー中心の視点が低い組織はスピードが仇となり、ユーザー価値に結びつかないコードや機能が増殖し、チームパフォーマンスは低下する。
...
1. 期待: AI の利用が推奨、あるいは必須とされているか
2. サポート: 実験のための時間やリソースが提供されているか
3. 許可: どのツールが承認されているか
4. 適用範囲: その方針が自身の役割にどう適用されるか
