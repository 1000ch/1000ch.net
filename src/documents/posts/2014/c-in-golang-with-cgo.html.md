...
最適化実行部分とコマンドラインのインターフェースがmain関数内にがっちり書かれていたので、今度はこれをどうにかしなければならならず、しばし考える。Cのmain関数で行っている処理をGoで再実装しようとも考えたのですが、長いし、一部だけexternするというのも微妙かと考え、main関数をリネームしGo側のmain関数からコールすることにする。ので、`jpegoptim.c.patch`も作りなおす。
...
終わってみればGoのコードは殆どなくなってしまったけど、何かの足しになればと思って記事として残します。こちらのビルドはリポジトリをクローンして、makeすればバイナリ作成まで出来るようになってます。Goのv1.3.2で確認済み。
